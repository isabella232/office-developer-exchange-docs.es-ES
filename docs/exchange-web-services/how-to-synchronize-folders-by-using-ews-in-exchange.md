...
<span data-ttu-id="82558-147">Si está utilizando la API administrada de EWS, después de obtener la lista de carpetas nuevas o modificadas, use el método [Folder.Load](http://msdn.microsoft.com/en-us/library/microsoft.exchange.webservices.data.folder.load%28v=exchg.80%29.aspx) para obtener las propiedades en los elementos nuevos o modificados, compare las propiedades con los valores locales y actualizar o crear las carpetas en el cliente.</span><span class="sxs-lookup"><span data-stu-id="82558-147">If you're using the EWS Managed API, after you get the list of new or changed folders, use the [Folder.Load](http://msdn.microsoft.com/en-us/library/microsoft.exchange.webservices.data.folder.load%28v=exchg.80%29.aspx) method to get properties on the new or changed items, compare the properties to the local values, and update or create the folders on the client.</span></span> 
